...
class WaypointMission:
        ...
        try:
            ...
            for i, waypoint in enumerate(self.waypoints):
                ...
                if is_last_waypoint:
                    ...
                    # 起飞点 - 第一次起飞用8秒
                    duration = 8.0 if is_first_waypoint else 2.0
                    self.fly_to_waypoint(x, y, z, f"起飞到航点{wp_id}", duration=duration)
                elif action == 'land':
                    # 降落点（中间的降落点仍按原逻辑处理）
                    self.land_drone(x, y)
                else:
                    # 普通巡航点 - 用2秒飞行时间
                    self.fly_to_waypoint(x, y, z, f"飞向航点{wp_id}", duration=2.0)
    ...
    def land_drone(self, x=None, y=None):
        ...
        if x is not None and y is not None:
            ...
            self.drone._send_position_x_y_z_t_frame(x, y, current_xyz[2], 2.0, frame="local")
