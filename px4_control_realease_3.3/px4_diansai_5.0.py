...
from std_msgs.msg import Int32, Bool  # 导入Bool消息类型用于接收扫描指令
...
class WaypointMission:
    def __init__(self, drone, params=None):
        ...
        # 扫描相关变量
        self.scan_requested = False  # 是否请求扫描
        self.scan_mask = 1  # 默认扫描掩码
        self.current_waypoint_index = 0  # 当前执行到的航点索引
        self.is_scanning = False  # 是否正在执行扫描
        
        # 参数设置
        self.params = {
            'first_waypoint_duration': 6.0,  # 第一个航点飞行时间(秒)
            'normal_waypoint_duration': 1.5,  # 普通航点飞行时间(秒)
            'landing_duration': 4.0,  # 降落时间(秒)
            'scan_point_duration': 0.12,  # 扫描点停留时间(秒)
            'scan_offset': 0.15,  # 扫描点偏移距离(米)
        }
        ...
        try:
            ...
            # 创建订阅器，监听扫描信号
            self.scan_sub = rospy.Subscriber('/sao_miao', Bool, self.scan_callback)
            # 创建发布器，发布扫描状态
            self.scan_status_pub = rospy.Publisher('/scan_status', Bool, queue_size=1)
    ...
    def scan_callback(self, msg):
        """
        处理来自cv_yolov8.py的扫描信号
        msg.data: True表示请求扫描当前区域
        """
        if msg.data and self.mission_active and not self.is_scanning:
            rospy.loginfo("收到扫描请求，将在当前区域进行扫描")
            self.scan_requested = True
            self.scan_mask = 1  # 设置为1表示需要进行扫描
    ...
    def execute_mission(self):
        ...
        try:
            ...
            while i < len(self.waypoints):
                ...
                # 检查是否收到扫描请求
                if self.scan_requested and self.scan_mask == 1:
                    rospy.loginfo(f"=== 在航点 {wp_id} 周围执行区域扫描 ===")
                    self.execute_area_scan(target_x, target_y, target_z)
                    self.scan_requested = False  # 重置扫描请求
    ...
    def execute_area_scan(self, center_x, center_y, center_z):
        """
        在当前区域执行扫描任务
        参数：
        - center_x, center_y, center_z: 区域中心坐标
        """
        try:
            self.is_scanning = True
            # 发布扫描开始状态
            self.scan_status_pub.publish(Bool(True))
            rospy.loginfo("发布扫描状态：开始扫描")
            
            scan_offset = self.params['scan_offset']
            scan_duration = self.params['scan_point_duration']
            
            # 定义4个扫描点的位置（顺时针：左上、右上、右下、左下）
            scan_points = [
                (center_x - scan_offset, center_y + scan_offset, center_z),  # 左上角
                (center_x + scan_offset, center_y + scan_offset, center_z),  # 右上角
                (center_x + scan_offset, center_y - scan_offset, center_z),  # 右下角
                (center_x - scan_offset, center_y - scan_offset, center_z)   # 左下角
            ]
            
            rospy.loginfo(f"开始执行区域扫描，共4个点，每点停留{scan_duration}秒，偏移距离{scan_offset}米")
            
            # 按顺时针顺序飞行4个点
            for i, (x, y, z) in enumerate(scan_points):
                point_name = ["左上角", "右上角", "右下角", "左下角"][i]
                rospy.loginfo(f"飞向扫描点{i+1}/4: {point_name} ({x:.3f}, {y:.3f}, {z:.3f})")
                
                # 发送位置控制指令
                self.drone.send_position_x_y_z_t_yaw_local(x, y, z, scan_duration, self.initial_yaw, use_thread=True)
                ...
                rospy.loginfo(f"到达扫描点{i+1}: 当前位置 ({current_xyz[0]:.2f}, {current_xyz[1]:.2f}, {current_xyz[2]:.2f})")
            
            # 完成扫描后返回区域中心
            rospy.loginfo("扫描完成，返回区域中心")
            self.drone.send_position_x_y_z_t_yaw_local(center_x, center_y, center_z, scan_duration, self.initial_yaw, use_thread=True)
            ...
            # 发布扫描结束状态
            self.scan_status_pub.publish(Bool(False))
            rospy.loginfo("发布扫描状态：扫描结束")
            
            self.is_scanning = False
            rospy.loginfo("区域扫描完成，继续执行原航点任务")
            
        except Exception as e:
            rospy.logerr(f"执行区域扫描失败: {e}")
            # 确保在异常情况下也发布扫描结束状态
            self.scan_status_pub.publish(Bool(False))
            self.is_scanning = False
...
def main():
    ...
    parser.add_argument('--normal-wp-time', type=float, default=1.88, help='普通航点飞行时间(秒)')
    parser.add_argument('--landing-time', type=float, default=4.0, help='降落时间(秒)')
    parser.add_argument('--scan-time', type=float, default=0.37, help='扫描点停留时间(秒)')
    parser.add_argument('--scan-offset', type=float, default=0.15, help='扫描点偏移距离(米)')
    ...
    params = {
        ...
        'scan_point_duration': args.scan_time,
    ...
    rospy.loginfo(f"扫描点停留时间: {params['scan_point_duration']}秒")
    rospy.loginfo(f"扫描点偏移距离: {params['scan_offset']}米")
    rospy.loginfo("================================")
