...
class WildlifeSurveyStation(QMainWindow):
    ...
    def __init__(self):
        ...
        # 创建订阅器 - 订阅目标检测结果（带区域ID的检测结果）
        rospy.Subscriber('/region_wildlife_detection', String, self.detection_callback, queue_size=10)
    ...
    def detection_callback(self, msg):
        ...
        try:
            ...
            distance_to_center = math.sqrt((drone_x - center_x)**2 + (drone_y - center_y)**2)
            
            # 检查是否符合检测条件:
            # 1. 无人机在0.23米容差范围内
            # 2. 进入新方格或距离上次检测时间超过3秒
            # 3. 当前方格之前未检测过
            if (distance_to_center <= 0.23 and
                ...
                if detection_data and detection_data != "wu_jiance_mubiao":
                    try:
                        ...
                        # 计算当前无人机所在的区域ID
                        current_region_id = f"B{grid_col+1} A{9-grid_row}"
                        
                        # 只有当视觉识别的区域ID与无人机当前区域ID一致时才统计
                        if vision_region_id == current_region_id:
                            # 更新当前方格位置和检测时间
                            ...
                            if detections:
                                ...
                                status_msg = f"在{grid_name}检测到: {animal_info} (区域匹配: {current_region_id})"
                            ...
                            # 区域ID不匹配，记录日志但不统计
                            rospy.loginfo(f"区域ID不匹配: 视觉识别区域={vision_region_id}, 无人机当前区域={current_region_id}")
                            
                    except json.JSONDecodeError:
                        # 如果不是JSON格式，尝试解析旧格式（向后兼容）
                        detections = {}
                        ...
                        if detections:
                            ...
                            animal_info = ", ".join([f"{name}: {count}只" for name, count in detections.items()])
                            grid_name = self.wildlife_detections[current_grid]['grid_coord']
                            status_msg = f"在{grid_name}检测到: {animal_info} (兼容模式)"
                            self.update_status_signal.emit(status_msg, 5000)
    ...
    def update_wildlife_display(self):
        ...
        # 构建显示文本
        html_text = "<html><body>"
        html_text += "<h3 style='font-size:12px;margin:5px 0;'>野生动物检测结果</h3>"
    ...
    def create_wildlife_panel(self):
        ...
        <html>
        <body>
        <h3 style='font-size:12px;'>野生动物检测结果</h3>
        <p style='color:#666;font-size:10px;'>当无人机到达方格中心时，将显示检测到的野生动物信息</p>
        </body>
        </html>
        """
